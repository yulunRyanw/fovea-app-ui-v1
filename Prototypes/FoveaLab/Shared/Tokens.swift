// Copied from Sources/Fovea/Tokens.swift for the lab. The lab may drift from the shipping file.
...
enum Tokens {
    ...
    enum Motion {
        ...
        static func animation(_ kind: Kind) -> Animation {
            .timingCurve(curve, duration: kind.duration * LabMotion.scale)
        }
        ...
        enum Spring {
            ...
            var animation: Animation {
                switch self {
                case .open: return .smooth(duration: 0.34 * LabMotion.scale)
                case .close: return .smooth(duration: 0.30 * LabMotion.scale)
                case .convert: return .smooth(duration: 0.28 * LabMotion.scale)
                case .hover: return .smooth(duration: 0.18 * LabMotion.scale)
                }
            }
        }
